...
import org.jsonk.JsonWriteException;

import static java.lang.Double.PRECISION;
import static java.lang.Double.doubleToRawLongBits;
import static java.lang.Long.numberOfLeadingZeros;
import static java.lang.Math.multiplyHigh;
import static org.jsonk.jdk.internal.MathUtils.*;
...
public final class DoubleToDecimal {
    ...
    private final char[] POS_ZERO_CHARS = {'0', '.', '0'};

    private final char[] NEG_ZERO_CHARS = {'-', '0', '.', '0'};
    ...
    public static int toDecimal(double v, char[] buf, int offset) {
        var d2d = new DoubleToDecimal(buf, offset);
        return switch (d2d.toDecimal(v)) {
            case NON_SPECIAL -> d2d.index - offset + 1;
            case PLUS_ZERO ->  {
                System.arraycopy(d2d.POS_ZERO_CHARS, 0, buf, offset, d2d.POS_ZERO_CHARS.length);
                yield 3;
            }
            case MINUS_ZERO -> {
                System.arraycopy(d2d.NEG_ZERO_CHARS, 0, buf, offset, d2d.NEG_ZERO_CHARS.length);
                yield 4;
            }
            default -> throw new JsonWriteException("Special double values are not supported: " + v);
        };
    }
    ...
    private DoubleToDecimal(char[] buf, int offset) {
        this.buf = buf;
        this.index = offset;
    }
