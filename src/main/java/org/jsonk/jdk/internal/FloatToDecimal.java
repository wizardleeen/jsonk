...
public final class FloatToDecimal {
    ...
    public static int toDecimal(float v, char[] buf, int offset) {
        var f2d = new FloatToDecimal(buf, offset);
        return switch (f2d.toDecimal(v)) {
            case NON_SPECIAL -> f2d.index - offset + 1;
            case PLUS_ZERO ->  {
                System.arraycopy(f2d.POS_ZERO_CHARS, 0, buf, offset, f2d.POS_ZERO_CHARS.length);
                yield 3;
            }
            case MINUS_ZERO -> {
                System.arraycopy(f2d.NEG_ZERO_CHARS, 0, buf, offset, f2d.NEG_ZERO_CHARS.length);
                yield 4;
            }
    ...
    private FloatToDecimal(char[] buf, int offset) {
        this.buf = buf;
        this.index = offset;
    }
